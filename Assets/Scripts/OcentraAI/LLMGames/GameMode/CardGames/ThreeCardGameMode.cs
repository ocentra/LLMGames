    ...
    public class ThreeCardGameMode : GameMode
    {
        ...
        protected override void InitializeGameRules()
        {
            ...
            GameRules.Player = $"{GameName} Rules:{Environment.NewLine}" +
                               ...
                               $"2. Players can bet blind (without seeing their hand) or see their hand before betting.{Environment.NewLine}" +
                               $"3. Blind betting doubles the current bet. Players who have seen their hand bet double the current bet.{Environment.NewLine}" +
                               $"4. Players can choose to fold, call, or raise at their turn.{Environment.NewLine}" +
                               $"5. Players can draw a new card to the floor.{Environment.NewLine}" +
                               $"6. Players can pick and swap a card from the floor. After picking and swapping, the player must decide to bet, fold, call, or raise.{Environment.NewLine}" +
                               $"7. The highest hand value wins (A high). In case of a tie, the highest card wins.{Environment.NewLine}" +
                               $"8. The game ends when a player runs out of coins or after a set number of rounds.{Environment.NewLine}" +
                               $"9. The trailing player can continue if they have more coins.{Environment.NewLine}" +
                               $"10. Players start with {InitialPlayerCoins} coins.{Environment.NewLine}" +
                               $"Special Rules for Three Card Brag:{Environment.NewLine}" +
                               $"11. Three of a Kind: Three cards of the same rank. Example: {GetRankSymbol(Suit.Diamond, Rank.J)}, {GetRankSymbol(Suit.Heart, Rank.J)}, {GetRankSymbol(Suit.Club, Rank.J)}{Environment.NewLine}" +
                               $"12. Royal Flush: Three cards in sequence of the same suit, starting from A. Example: {GetRankSymbol(Suit.Spade, Rank.A)}, {GetRankSymbol(Suit.Spade, Rank.K)}, {GetRankSymbol(Suit.Spade, Rank.Q)}{Environment.NewLine}" +
                               $"13. Straight Flush: Three cards in sequence of the same suit. Example: {GetRankSymbol(Suit.Spade, Rank.Nine)}, {GetRankSymbol(Suit.Spade, Rank.Nine)} ,  {GetRankSymbol(Suit.Spade, Rank.Nine)}{Environment.NewLine}" +
                               $"14. Straight: Three cards in sequence but not all of the same suit. Example: {GetRankSymbol(Suit.Spade, Rank.Four)}, {GetRankSymbol(Suit.Club, Rank.Five)}, {GetRankSymbol(Suit.Diamond, Rank.Six)}{Environment.NewLine}" +
                               $"15. Flush: Three cards of the same suit but not in sequence. Example: {GetRankSymbol(Suit.Spade, Rank.Two)} ,  {GetRankSymbol(Suit.Spade, Rank.Five)}, {GetRankSymbol(Suit.Spade, Rank.Nine)}{Environment.NewLine}" +
                               $"16. Pair: Two cards of the same rank. Example: {GetRankSymbol(Suit.Spade, Rank.Q)}, {GetRankSymbol(Suit.Diamond, Rank.Q)}, {GetRankSymbol(Suit.Club, Rank.Seven)}{Environment.NewLine}" +
                               $"17. High Card: When no other hand is achieved, the highest card plays. Example: {GetRankSymbol(Suit.Spade, Rank.A)}, {GetRankSymbol(Suit.Diamond, Rank.Seven)}, {GetRankSymbol(Suit.Club, Rank.Four)} (A high){Environment.NewLine}" +
                               $"18. Trump Card: A designated card that can replace any card to complete a sequence or set. Example: Trump card is {GetRankSymbol(Suit.Heart, Rank.Six)}. Hand: {GetRankSymbol(Suit.Spade, Rank.Two)}, {GetRankSymbol(Suit.Spade, Rank.Three)}, {GetRankSymbol(Suit.Heart, Rank.Six)} (Trump card converts to {GetRankSymbol(Suit.Spade, Rank.Four)} to make straight flush: {GetRankSymbol(Suit.Spade, Rank.Two)}, {GetRankSymbol(Suit.Spade, Rank.Three)}, {GetRankSymbol(Suit.Spade, Rank.Four)}){Environment.NewLine}" +
                               $"19. Trump in Middle: The Trump card is considered 'in the middle' if it is surrounded by cards on either side that form a continuous sequence. Example: {GetRankSymbol(Suit.Spade, Rank.Five)}, {GetRankSymbol(Suit.Heart, Rank.Six)}, {GetRankSymbol(Suit.Spade, Rank.Seven)} ({GetRankSymbol(Suit.Heart, Rank.Six)} Trump card is in the middle){Environment.NewLine}" +
        ...
        protected override void InitializeGameDescription()
        {
            ...
            GameDescription.Player =
                $"Three Card Brag is an exciting card game that combines elements of poker and bluffing.{Environment.NewLine}" +
        ...
        protected override void InitializeStrategyTips()
        {
            ...
            StrategyTips.Player = $"Pay attention to your opponents' betting patterns.{Environment.NewLine}" +
