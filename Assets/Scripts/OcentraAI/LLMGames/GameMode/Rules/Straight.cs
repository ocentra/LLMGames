...
{
    [CreateAssetMenu(fileName = nameof(Straight), menuName = "Rules/Straight")]
    public class Straight : BaseBonusRule
    {
        public override string RuleName { get; protected set; } = $"{nameof(Straight)}";
        ...
        public override bool Evaluate(List<Card> hand, out BonusDetails bonusDetails)
        {
            ...
            // Check for trump-assisted straight
            if (GameMode.UseTrump)
            {
                ...
                if (hasTrumpCard)
                {
                    ...
                    bool canFormStraight = CanFormSequenceWithWild(nonTrumpCards.Select(c => c.GetRankValue()).ToList());
        ...
        private BonusDetails CalculateBonus(List<Card> hand, bool isTrumpAssisted)
        {
            ...
            var descriptions = new List<string> { $"Straight:" };

            if (isTrumpAssisted)
            {
                additionalBonus += GameMode.TrumpBonusValues.StraightBonus;
                descriptions.Add($"Trump Card Bonus: +{GameMode.TrumpBonusValues.StraightBonus}");
        ...
        {
            RuleName = "Straight Rule";
            Description = "A sequence of cards in rank, optionally considering Trump Wild Card.";
            ...
            Description = $"A sequence of cards from {fromRank} to {toRank}.";
