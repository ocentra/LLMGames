    ...
    public class ThreeCardGameMode : GameMode
    {
        ...
        protected override void InitializeGameRules()
        {
            ...
            GameRules.Player = $"{GameName} Rules:{System.Environment.NewLine}" +
                               ...
                               $"11. Three of a Kind: Three cards of the same rank. Example: {GetRankSymbol(Suit.Diamonds, Rank.J)}, {GetRankSymbol(Suit.Hearts, Rank.J)}, {GetRankSymbol(Suit.Clubs, Rank.J)}{System.Environment.NewLine}" +
                               $"12. Royal Flush: Three cards in sequence of the same suit, starting from A. Example: {GetRankSymbol(Suit.Spades, Rank.A)}, {GetRankSymbol(Suit.Spades, Rank.K)}, {GetRankSymbol(Suit.Spades, Rank.Q)}{System.Environment.NewLine}" +
                               $"13. Straight Flush: Three cards in sequence of the same suit. Example: {GetRankSymbol(Suit.Spades, Rank.Nine)}, {GetRankSymbol(Suit.Spades, Rank.Ten)}, {GetRankSymbol(Suit.Spades, Rank.J)}{System.Environment.NewLine}" +
                               $"14. Straight: Three cards in sequence but not all of the same suit. Example: {GetRankSymbol(Suit.Spades, Rank.Four)}, {GetRankSymbol(Suit.Clubs, Rank.Five)}, {GetRankSymbol(Suit.Diamonds, Rank.Six)}{System.Environment.NewLine}" +
                               $"15. Flush: Three cards of the same suit but not in sequence. Example: {GetRankSymbol(Suit.Spades, Rank.Two)}, {GetRankSymbol(Suit.Spades, Rank.Five)}, {GetRankSymbol(Suit.Spades, Rank.Nine)}{System.Environment.NewLine}" +
                               $"16. Pair: Two cards of the same rank. Example: {GetRankSymbol(Suit.Spades, Rank.Q)}, {GetRankSymbol(Suit.Diamonds, Rank.Q)}, {GetRankSymbol(Suit.Clubs, Rank.Seven)}{System.Environment.NewLine}" +
                               $"17. High Card: When no other hand is achieved, the highest card plays. Example: {GetRankSymbol(Suit.Spades, Rank.A)}, {GetRankSymbol(Suit.Diamonds, Rank.Seven)}, {GetRankSymbol(Suit.Clubs, Rank.Four)} (A high){System.Environment.NewLine}" +
                               $"18. Trump Card: A designated card that can replace any card to complete a sequence or set. Example: Trump card is {GetRankSymbol(Suit.Hearts, Rank.Six)}. Hand: {GetRankSymbol(Suit.Spades, Rank.Two)}, {GetRankSymbol(Suit.Spades, Rank.Three)}, {GetRankSymbol(Suit.Hearts, Rank.Six)} (Trump card converts to {GetRankSymbol(Suit.Spades, Rank.Four)} to make straight flush: {GetRankSymbol(Suit.Spades, Rank.Two)}, {GetRankSymbol(Suit.Spades, Rank.Three)}, {GetRankSymbol(Suit.Spades, Rank.Four)}){System.Environment.NewLine}" +
                               $"19. Trump in Middle: The Trump card is considered 'in the middle' if it is surrounded by cards on either side that form a continuous sequence. Example: {GetRankSymbol(Suit.Spades, Rank.Five)}, {GetRankSymbol(Suit.Hearts, Rank.Six)}, {GetRankSymbol(Suit.Spades, Rank.Seven)} ({GetRankSymbol(Suit.Hearts, Rank.Six)} Trump card is in the middle){System.Environment.NewLine}" +
                               $"20. Adjacent to Trump: Cards adjacent to the Trump card provide a special point bonus, even if they do not form a valid hand. Example: {GetRankSymbol(Suit.Spades, Rank.Two)}, {GetRankSymbol(Suit.Spades, Rank.Three)}, {GetRankSymbol(Suit.Hearts, Rank.Seven)} ({GetRankSymbol(Suit.Hearts, Rank.Seven)} is adjacent to the Trump card {GetRankSymbol(Suit.Hearts, Rank.Six)} and receives points, but the hand does not form a valid sequence).";
